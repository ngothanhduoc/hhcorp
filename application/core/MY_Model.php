...
class MY_Model extends CI_Model {
    ...
    function count_row_table($where = "") {
        $this->db_slave->select("count(*) total");
        if (!empty($where))
            $sql = $this->db_slave->get_where($this->_table, $where);
        else
    ...
    function get_table($start = 0, $limit = 0) {
        if ($limit === 0) {
            ...
            $sql = $this->db_slave->limit($limit, $start)->get($this->_table);
        }
